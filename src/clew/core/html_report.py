...
.span.skipped { border-color: var(--warning); }
.span-head {
...
.status-skipped { color: var(--warning); font-size: 0.78em; }
...
{html_tree}
  ...
  <footer>
    <span>Generated by <a href="https://github.com/samsamurai301/clew-ai">Clew</a></span>
...
def _status_class(s: Span) -> str:
    ...
    if s.status is SpanStatus.ERROR:
        return "status-error"
    if s.status is SpanStatus.SKIPPED:
...
def _render_span(s: Span) -> str:
    ...
    err_class = (
        " error"
        if s.status is SpanStatus.ERROR
        else (" skipped" if s.status is SpanStatus.SKIPPED else "")
    )
    ...
    parts.append(f'<div class="label">id</div><pre>{html.escape(s.id)}</pre>')
    if s.parent_ids:
        parts.append(
            f'<div class="label">parent_ids</div><pre>{html.escape(", ".join(s.parent_ids))}</pre>'
        )
    if s.error:
        parts.append(f'<div class="label">error</div><pre>{html.escape(s.error)}</pre>')
    if s.attributes:
        parts.append(
    ...
    if s.input is not None:
        parts.append(
            f'<div class="label">input</div><pre>{html.escape(_format_value(s.input))}</pre>'
        )
    if s.output is not None:
        parts.append(
            f'<div class="label">output</div><pre>{html.escape(_format_value(s.output))}</pre>'
        )
    if s.metadata:
        parts.append(
            f'<div class="label">metadata</div><pre>{html.escape(_format_value(s.metadata))}</pre>'
        )
...
def _compute_max_depth(trace: Trace) -> int:
    """Return maximum DAG depth iteratively, counting each occurrence once."""
    if not trace.spans:
        return 0
    children: dict[str, list[str]] = defaultdict(list)
    in_degree = {span.id: len(span.parent_ids) for span in trace.spans}
    depth = {span.id: 1 for span in trace.spans if not span.parent_ids}
    ready = [span.id for span in trace.spans if not span.parent_ids]
    ...
    while ready:
        ...
        for child_id in children[parent_id]:
            depth[child_id] = max(depth.get(child_id, 1), depth[parent_id] + 1)
            in_degree[child_id] -= 1
            if in_degree[child_id] == 0:
                ready.append(child_id)
    # ``Trace`` already rejects cycles and missing parents. Keep this helper
    # total for callers constructing subclasses or future model variants.
    return max((depth.get(span_id, 1) for span_id in by_id), default=0)
...
def render_html(trace: Trace) -> str:
    ...
    for s in trace.spans:
        if s.parent_ids:
            # A card is rendered once under its first ordered parent. Every
            # parent remains visible in the card details, avoiding exponential
            # path expansion for legitimate multi-parent DAGs.
            children[s.parent_ids[0]].append(s)
    # Order children by started_at for stability.
    for k in children:
        children[k].sort(key=lambda s: s.started_at or _MIN)

    def _render_tree(span_id: str) -> str:
        parts: list[str] = []
        stack: list[tuple[str, str]] = [("enter", span_id)]
        while stack:
            action, current_id = stack.pop()
            if action == "close":
                parts.append("</li>")
                continue
            if action == "close-children":
                parts.append("</ul>")
                continue
            parts.append(_render_span(by_id[current_id]))
            kids = children.get(current_id, [])
            stack.append(("close", current_id))
            if kids:
                parts.append("<ul>")
                stack.append(("close-children", current_id))
                for child in reversed(kids):
                    stack.append(("enter", child.id))
        return "\n".join(parts)

    root = next((s for s in trace.spans if s.id == trace.root_span_id), None)
    if root is None and trace.spans:
        root = trace.spans[0]
    html_tree = "<li><em>(empty trace)</em></li>" if root is None else _render_tree(root.id)
